...
int COFSquadTalkMonster::SquadRecruit(int searchRadius, int maxMembers)
{
	...
	if (!FStringNull(pev->netname))
	{
		...
		while (pEntity)
		{
			COFSquadTalkMonster* pRecruit = (COFSquadTalkMonster*)((CBaseMonster*)pEntity);
			bool sameEntityclass = FStrEq(STRING(pev->classname), STRING(pRecruit->pev->classname)); // Make sure we have the same entity class

			if (pRecruit && sameEntityclass)
			{
				if (!pRecruit->InSquad() && pRecruit->Classify() == iMyClass && pRecruit != this && pRecruit->m_uiFactionID == m_uiFactionID)
				{
	...
	else
	{
		...
		{
			COFSquadTalkMonster* pRecruit = (COFSquadTalkMonster*)pEntity;
			bool sameEntityclass = FStrEq(STRING(pev->classname), STRING(pRecruit->pev->classname));

			if (pRecruit && pRecruit != this && pRecruit->IsAlive() && !pRecruit->m_pCine && sameEntityclass)
			{
				// Can we recruit this guy?
				if (!pRecruit->InSquad() && pRecruit->Classify() == iMyClass &&
					((iMyClass != CLASS_ALIEN_MONSTER) || sameEntityclass) &&
					FStringNull(pRecruit->pev->netname) && pRecruit->m_uiFactionID == m_uiFactionID)
				{
					TraceResult tr;
...
void COFSquadTalkMonster::FollowerUse(CBaseEntity* pActivator, CBaseEntity* pCaller, USE_TYPE useType, float value)
{
	...
	if (pCaller != NULL && pCaller->IsPlayer())
	{
		...
		else if (CanFollow())
		{
			...
			if ((m_afMemory & bits_MEMORY_PROVOKED) != 0 || m_uiFactionID != -1)
...
COFSquadTalkMonster* COFSquadTalkMonster::FindSquadMedic(int searchRadius)
{
	...
	{
		auto pMonster = (COFSquadTalkMonster*)pEntity;
