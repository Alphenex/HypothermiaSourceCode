...
void CEnvFog::Spawn(void)
{
	...
	if (FBitSet(pev->spawnflags, 1) || FStringNull(pev->targetname))
		m_bActive = true;
	else
		m_bActive = false;
}
...
void CEnvFog::Use(CBaseEntity* pActivator, CBaseEntity* pCaller, USE_TYPE useType, float value)
{
	...
	if (prevState != m_bActive)
	{
		...
		if (m_bActive || !(bool)m_flBlendTime)
		{
...
{
	if ((!(bool)blendtime || g_iCurrentEndDist < enddist) || !(bool)g_iCurrentEndDist)
	{
...
{
	if (!(bool)g_flBlendDoneTime || !(bool)g_iIdealEndDist)
		return;
