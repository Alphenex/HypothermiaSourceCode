...
{
	// If we use TriApi's Fog function it breaks when using shaders but fixes transparency when not using shaders.
	// And if we don't use the TriApi's Fog function then it works for both but transparency is fucked.
	...
	glEnable(GL_FOG); // Enable Fog Mode
